    ...
    const interval = setInterval(() => {
      if (isMounted.current && flatListRef.current) {
        setActiveIndex((prevIndex) => {
          const nextIndex =
            prevIndex + 1 >= comicBannerData.length ? 0 : prevIndex + 1;
        ...
        <View className="w-full h-[240px] bg-zinc-900/50 border border-white/5 rounded-[28px] p-4 flex-row items-center shadow-2xl">
          ...
          <View className="flex-1 ml-5 h-[200px] justify-between py-1">
            <View>
              ...
              <Text
                className="text-white text-xl Hong font-extrabold tracking-wide leading-7"
                numberOfLines={2}
              >
                {item.title}
              </Text>
              ...
              <Text
                className="text-[#7C766B] text-sm font-medium mt-2 leading-5"
                numberOfLines={2}
              >
                {item.subtitle}
              </Text>
            ...
            <TouchableOpacity className="flex-row h-11 bg-[#D4AF37] rounded-2xl items-center justify-center self-start px-6 shadow-md active:opacity-80">
